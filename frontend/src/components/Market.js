                    ...
                    <tbody>
                        {footballers.map((item, index) => (
                            <tr className="table-row" key={`${index}-row`}>
                                ...
                                <td className="text-end"><button className={`btn btn-primary ${isAuthenticated ? '' : 'd-none'}`} onClick={() => processSell({name: item.footballer.name, price: item.sell_price})}>SELL {item.sell_price.toFixed(2)}</button></td>
                                <td className="text-end"><button className={`btn btn-success ${isAuthenticated ? '' : 'd-none'}`} onClick={() => processBuy({name: item.footballer.name, price: item.buy_price})}>BUY {item.buy_price.toFixed(2)}</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
